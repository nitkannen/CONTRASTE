...
def decode_pred_triplets(text):
    ...
    for s in text_processed.split(" [SSEP] "):
        if "<aspect>" in s and "<category>" in s and "<sentiment>" in s:
            at = s.split("<category>")[0].split("<aspect>")[1].strip()
            ot = s.split("<category>")[1].split("<sentiment>")[0].strip()
            sp = s.split("<sentiment>")[1].strip()
        else:
            # print(f'Cannot decode: {s}')
            at, ot, sp = "", "", ""

        entry = {"aspect": at.strip(), "category": ot.strip(), "sentiment": sp.strip()}
...
def is_full_match(triplet, triplets):
    for t in triplets:
        if (
            ...
            and t["category"].lower() == triplet["category"].lower()
...
def get_f1_for_trainer(predictions, target, component=None):
    ...
    for i in range(n):
        ...
        if component == "aspect":
            ...
            correct_count += len(
                list(set(pred_opinions).intersection(set(gold_opinions)))
            )
