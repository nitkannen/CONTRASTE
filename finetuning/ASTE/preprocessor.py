...
def generate_target(d):
    """
    Take aspect triple dictionary and linearizes it
    """
    summary = ""
    if len(d) == 0:
        return summary
    ...
    for items in d:
        triplets.append(
            f"<aspect> {items[0]} <opinion> {items[1]} <sentiment> {sent_map[items[2]]}"
        )
...
def generate_triplet_dict(tuples, sentence):
    """
    Take set of tuples and generates triplet dictionary.
    """
    ...
    for triplet in triplets:
        ...
        ordered_triplets.append(
            (sentence.find(a.strip()), sentence.find(o.strip()), triplet)
        )
